    ...
    try {
      ...
      if (data.user && data.session) {
        ...
        if (isServiceRoleConfigured) {
          try {
            ...
            if (!existingProfile) {
              ...
              if (profileError) {
                ...
                console.log(
                  "User profile created successfully after email verification"
                );
              }
            }
          } catch (profileError) {
            console.error(
              "Error setting up admin client or creating profile:",
              profileError
            );
          ...
          console.warn(
            "SUPABASE_SERVICE_ROLE_KEY not configured, skipping profile creation. User can still sign in."
          );
        ...
        if (data.user.email_confirmed_at) {
          ...
          console.log(
            "[OAuth Callback] User verified, redirecting to dashboard map"
          );
