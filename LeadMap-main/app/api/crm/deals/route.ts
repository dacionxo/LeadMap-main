...
function normalizeStage(stage: string | null | undefined): string {
  ...
  if (normalized.includes("closed") && normalized.includes("won"))
    return "closed_won";
  if (normalized.includes("closed") && normalized.includes("lost"))
    return "closed_lost";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const dealsWithListingIds = (deals || []).filter(
      (deal: any) => deal.listing_id
    );
    const uniqueListingIds = Array.from(
      new Set(dealsWithListingIds.map((deal: any) => deal.listing_id))
    );

    // Build maps: listing_id -> property_address, list_price, photos_json, primary_photo, property_class
    const propertyAddressMap = new Map<string, string | null>();
    const propertyValueMap = new Map<string, number | null>();
    const propertyPhotosMap = new Map<
      string,
      { photos_json: any; primary_photo: string | null }
    >();
    const propertyClassMap = new Map<string, string | null>();
    ...
    if (uniqueListingIds.length > 0) {
      ...
      for (const tableName of tableNames) {
        try {
          const colsWithPrice =
            "listing_id, street, city, state, zip_code, property_url, list_price, property_class";
          const colsWithPhotos =
            "listing_id, street, city, state, zip_code, property_url, list_price, photos_json, property_class";
          let listings: any[] = [];
          ...
          if (
            errId &&
            (errId.code === "42703" ||
              /photos_json|list_price|property_class|column.*exist/i.test(
                errId.message || ""
              ))
          ) {
          ...
          if (listings.length > 0) {
            listings.forEach((listing: any) => {
              ...
              if (!id) return;
              if (!propertyAddressMap.has(id))
                propertyAddressMap.set(id, buildAddress(listing));
              const pv =
                listing.list_price != null ? Number(listing.list_price) : null;
              if (pv != null && !isNaN(pv) && !propertyValueMap.has(id))
                propertyValueMap.set(id, pv);
              if (
                typeof listing.property_class === "string" &&
                !propertyClassMap.has(id)
              ) {
                propertyClassMap.set(id, listing.property_class);
              }
              if (listing.photos_json != null && !propertyPhotosMap.has(id)) {
                const primary = getFirstPhotoUrl(listing.photos_json);
                propertyPhotosMap.set(id, {
          ...
          const urlListingIds = uniqueListingIds.filter(
            (id: string) => id && id.startsWith("http")
          );
          if (urlListingIds.length > 0) {
            const { data: listingsByUrl, error: errUrl } = await supabase
              .from(tableName)
              .select(
                "listing_id, property_url, street, city, state, zip_code, list_price, photos_json, property_class"
              )
            ...
            if (
              ...
              const { data: fallback } = await supabase
                .from(tableName)
                .select(
                  "listing_id, property_url, street, city, state, zip_code, list_price, property_class"
                )
                .in("property_url", urlListingIds)
                .limit(1000);
              byUrl = fallback || [];
            ...
            if (errUrl && byUrl.length === 0) {
              const { data: fallback2 } = await supabase
                .from(tableName)
                .select(
                  "listing_id, property_url, street, city, state, zip_code"
                )
                .in("property_url", urlListingIds)
                .limit(1000);
            ...
            if (byUrl.length > 0) {
              byUrl.forEach((listing: any) => {
                ...
                if (!url) return;
                if (!propertyAddressMap.has(url))
                  propertyAddressMap.set(url, buildAddress(listing));
                const pv =
                  listing.list_price != null
                    ? Number(listing.list_price)
                    : null;
                if (pv != null && !isNaN(pv) && !propertyValueMap.has(url))
                  propertyValueMap.set(url, pv);
              if (
                typeof listing.property_class === "string" &&
                !propertyClassMap.has(url)
              ) {
                propertyClassMap.set(url, listing.property_class);
              }
                if (
                  listing.photos_json != null &&
                  !propertyPhotosMap.has(url)
                ) {
                  const primary = getFirstPhotoUrl(listing.photos_json);
                  propertyPhotosMap.set(url, {
                    photos_json: listing.photos_json,
                    primary_photo: primary,
                  });
                }
    ...
    const dealsWithOwnerIds = (deals || []).filter(
      (deal: any) => deal.owner_id
    );
    const uniqueOwnerIds = Array.from(
      new Set(dealsWithOwnerIds.map((deal: any) => deal.owner_id))
    );
    ...
    // Map property address, property value (list_price), photos, property_class, and owner back to deals
    const dealsWithProperties = (deals || []).map((deal: any) => {
      const result: any = { ...deal };

      if (deal.listing_id) {
        result.property_address =
          propertyAddressMap.get(deal.listing_id) || null;
        result.property_value = propertyValueMap.get(deal.listing_id) ?? null;
        result.property_class =
          propertyClassMap.get(deal.listing_id) ?? null;
